...
async function load() {
    ...
    // fitur 2 menampilkan jadwal pada kalender
...
function renderActivityAndTodayBoxes() {
    ...
    eventsInRange.forEach((e) => {
        ...
        eventDiv.innerHTML = `
            ...
            <div style="display:flex; gap:8px; align-items:center; flex-shrink:0;">
                ...
                <span class="status" style="background-color: ${color}; color: white; padding: 4px 8px; border-radius: 6px; font-size: 12px;">${status}</span>
            </div>
        `;
    ...
    try {
        ...
        if (activityCountEl) activityCountEl.textContent = String(eventsInRange.length);
    } catch (e) { console.warn('Gagal memperbarui activity count:', e); }
    ...
    if (activityBox.querySelectorAll('.tugas').length === 0) activityBox.innerHTML += `<p style="color:#666; padding-top: 10px;">Tidak ada jadwal/tugas terdekat.</p>`;
    if (todayBox.querySelectorAll('.jadwal').length === 0) todayBox.innerHTML += `<p style="color:#666; padding-top: 10px;">Tidak ada jadwal hari ini</p>`;
        ...
        eventsForDay.forEach(eventForDay => {
            ...
            eventListHTML += `
                <div class="single-event-detail" style="border: 1px solid #ccc; padding: 10px; margin-bottom: 10px; border-radius: 5px;">
                    <p style="margin: 0;"><strong>${eventForDay.title}</strong><br>Waktu: ${eventForDay.time}<br>Ruangan: ${eventForDay.room || '-'}<br>Dosen: ${dosenName}</p>
                    <button class="delete-single-btn" style="margin-top: 8px; padding: 5px 10px; cursor: pointer; background-color: #d9534f; color: white; border: none; border-radius: 3px;"
                        onclick="confirmDeleteEvent(${eventForDay.id}, '${eventForDay.repeatId || 'null'}', '${eventForDay.title}')">Hapus Jadwal</button>
                </div>`;
        });
...
async function saveEvent() {
    ...
    try {
        ...
        else alert("Gagal menyimpan ke server: " + result.message); 
    } catch (error) { alert('Gagal berkomunikasi dengan server.'); }
...
async function deleteEvent() {
    if (!clickedEventId && !clickedRepeatId) { alert("ID jadwal tidak valid."); return; }
    let msg = clickedRepeatId ? `Hapus SEMUA jadwal '${clickedEventTitle}' yang berulang?` : `Hapus jadwal '${clickedEventTitle}'?`;
    if (!confirm(msg)) return;
    try {
        const response = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'delete_event', id: clickedEventId, repeatId: clickedRepeatId })
        });
        const result = await response.json();
        if (result.status === 'success') alert(result.message);
...
function initButtons() {
    ...
    document.getElementById("addSubjectBtn").addEventListener("click", function () {
        ...
        if (isNaN(jumlah) || jumlah < 0) { alert("Input tidak valid!"); return; }
